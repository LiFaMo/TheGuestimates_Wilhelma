...
def load_superhero_domain():
    ...
    superhero_nlu = HandcraftedNLU(domain=domain)
    superhero_bst = HandcraftedBST(domain=domain)
    superhero_policy = HandcraftedPolicy(domain=domain)
    superhero_nlg = load_nlg(backchannel=False, domain=domain)
    return domain, [superhero_nlu, superhero_bst, superhero_policy, superhero_nlg]

def load_wilhelma_domain():
    from utils.domain.jsonlookupdomain import JSONLookupDomain
    from services.nlu.nlu import HandcraftedNLU
    from services.nlg.nlg import HandcraftedNLG
    from services.policy import HandcraftedPolicy
    domain = JSONLookupDomain(name='wilhelma',
                                json_ontology_file='resources/ontologies/wilhelma.json',
                                sqllite_db_file='resources/databases/wilhelma.db',
                                display_name='Wilhelma Domain')
    wilhelma_nlu = HandcraftedNLU(domain=domain)
    wilhelma_bst = HandcraftedBST(domain=domain)
    wilhelma_policy = HandcraftedPolicy(domain=domain)
    wilhelma_nlg = load_nlg(backchannel=False, domain=domain)
    return domain, [wilhelma_nlu, wilhelma_bst, wilhelma_policy, wilhelma_nlg]
...
if __name__ == "__main__":
    ...
    parser.add_argument('domains', nargs='+', choices=['lecturers', 'weather', 'mensa', 'qa', 'superhero', 'wilhelma'],
    ...
    if 'wilhelma' in args.domains:
        wilhelma_domain, wilhelma_services = load_wilhelma_domain()
        domains.append(wilhelma_domain)
        services.extend(wilhelma_services)
